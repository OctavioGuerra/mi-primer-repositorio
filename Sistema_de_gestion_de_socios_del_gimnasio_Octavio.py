...
def agregar_clases(clases_del_socio):
    return clases_del_socio >= 0
...
def agregar_socio(Datos):
    ...
    if not agregar_nombre(nom_del_socio):
        ...
        return
    clases_del_socio=int(input("Ingrese la cantidad de clases: "))
    if not agregar_clases(clases_del_socio):
        print("Dato invalido")
        return
    ...
    inf_usuario={
        ...
        "clases_disponibles":clases_del_socio,
...
while opc!=6:
    ...
    if opc==1:
        try:
            ...
            print("Error, ingrese el valor valido.")
    elif opc==2:
        nom_buscar=input("ingrese el nombre del socio: ")
        posicion=buscar_socio(Datos, nom_buscar)
        if posicion !=-1:
            print("Socio encontrado")
            print("Nombre:", Datos[posicion]["nombre"])
            print("Clases:", Datos[posicion]["clases_disponibles"])
            print("Mensualidad:", Datos[posicion]["mensualidad"])
        else:
